...
# LLC = 4 HNF × 5MiB = 20MiB ; aggressor = 1 × LLC(total) = 20MB each
...
OUT=$ROOT/logs/final_hotstorage_results_8592_agg1x
PCTS="10 25 53 100"
...
OUT  = ROOT / "logs" / "final_hotstorage_results_8592_agg1x"
TSV  = OUT / "results_final_hotstorage_8592_agg1x.tsv"
LABEL = "8592  4cpu agg1x (final)"
LLC_KIB = 20480
def stat(d, key):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8592+ final HotStorage (4 CPU, LLC=20MiB, agg=1×LLC, all jobs at once) ====="
