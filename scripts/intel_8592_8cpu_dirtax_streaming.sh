...
ITERS=20971520      # max_vs(40960KiB) × 256 × 2 passes
...
LABEL = "8592  8cpu dirtax_streaming (agg2x)"
LLC_KIB = 40960
NAGG = 7
def stat(d, key):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8592+ DirTax+STREAMING (8 CPU, LLC=40MiB, agg=2×LLC, all jobs at once) ====="
