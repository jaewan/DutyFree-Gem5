#!/usr/bin/env bash
# Intel Xeon 8592+ (Emerald Rapids) — 4 CPU final HotStorage results
# LLC = 4 HNF × 5MiB = 20MiB ; aggressor = 3 × LLC(total) = 60MB each
...
OUT=$ROOT/logs/final_hotstorage_results_8592_agg3x
PCTS="10 25 53 100"
...
OUT  = ROOT / "logs" / "final_hotstorage_results_8592_agg3x"
TSV  = OUT / "results_final_hotstorage_8592_agg3x.tsv"
LABEL = "8592  4cpu agg3x (final)"
LLC_KIB = 20480
def stat(d, key):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8592+ final HotStorage (4 CPU, LLC=20MiB, agg=3×LLC, all jobs at once) ====="
