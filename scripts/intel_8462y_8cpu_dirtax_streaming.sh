#!/usr/bin/env bash
# Intel Xeon 8462Y+ (Sapphire Rapids) — 8 CPU: DirTax + STREAMING (combined, agg2x)
...
# NOTE: 8 CPU will not run with small memory → mem 512GiB / cxl 256GiB.
#       assumes store-fix aggressor build; GEM5 build with the prefetch STREAMING patch recommended.
...
ITERS=8388608       # max_vs(16384KiB) × 256 × 2 passes
...
LABEL = "8462Y  8cpu dirtax_streaming (agg2x)"
LLC_KIB = 16384
NAGG = 7
def stat(d, key):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8462Y+ DirTax+STREAMING (8 CPU, LLC=16MiB, agg=2×LLC, all jobs at once) ====="
