#!/usr/bin/env bash
# Intel Xeon 8462Y+ (Sapphire Rapids) — 4 CPU: DirTax + STREAMING (combined, agg2x)
# LLC = 4 HNF × 2MiB = 8MiB ; aggressor = 2 × LLC(total) = 16MB each
...
OUT=$ROOT/logs/intel_8462y_4cpu_dirtax_streaming
...
OUT  = ROOT / "logs" / "intel_8462y_4cpu_dirtax_streaming"
TSV  = OUT / "results_intel_8462y_4cpu_dirtax_streaming.tsv"
LABEL = "8462Y  4cpu dirtax_streaming (agg2x)"
LLC_KIB = 8192
NAGG = 3
def stat(d, key):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8462Y+ DirTax+STREAMING (4 CPU, LLC=8MiB, agg=2×LLC, all jobs at once) ====="
    ...
    for pct in $PCTS; do
        ...
        env RUBY_RANDOMIZATION=1 $GEM5 --outdir=$OUT/with_streaming_${tag} \
            $COMMON -c "$V;$SA;$SA;$SA" --options "$vs $ITERS;$AGG_MB stream;$AGG_MB stream;$AGG_MB stream" \
