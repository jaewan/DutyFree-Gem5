#!/usr/bin/env bash
# Intel Xeon 8462Y+ (Sapphire Rapids) — 4 CPU: DirTax + STREAMING (combined)
# LLC = 4 HNF × 2MiB = 8MiB ; aggressor = 4 × LLC(total) = 32MB each
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8462Y+ DirTax+STREAMING (4 CPU, LLC=8MiB, agg=4×LLC, max ${MAXJOBS} jobs) ====="
