#!/usr/bin/env bash
# Intel Xeon 8462Y+ (Sapphire Rapids) — 2 CPU: DirTax + STREAMING (combined)
# LLC = 2 HNF × 2MiB = 4MiB ; aggressor = 4 × LLC(total) = 16MB each
# victim → DRAM 150ns, aggressor → CXL 300ns
#
# Three run types per victim WSS%:
#   alone            victim + dummy×1
#   with_agg         victim + WB aggressor×1       (dirtax/aggressor   → LLC fill)
#   with_streaming   victim + STREAMING aggressor×1 (dutyfree/aggressor → LLC bypass)
#
...
OUT=$ROOT/logs/intel_8462y_2cpu_dirtax_streaming
...
OUT  = ROOT / "logs" / "intel_8462y_2cpu_dirtax_streaming"
TSV  = OUT / "results_intel_8462y_2cpu_dirtax_streaming.tsv"
LABEL = "8462Y  2cpu"
LLC_KIB = 4096
def ticks(d):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8462Y+ DirTax+STREAMING (2 CPU, LLC=4MiB, agg=4×LLC, max ${MAXJOBS} jobs) ====="
    ...
    for pct in $PCTS; do
        ...
        $GEM5 --outdir=$OUT/with_streaming_${tag} \
            $COMMON -c "$V;$SA" --options "$vs $ITERS;$AGG_MB" \
