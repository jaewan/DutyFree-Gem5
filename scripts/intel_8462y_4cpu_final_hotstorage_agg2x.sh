...
OUT=$ROOT/logs/final_hotstorage_results_8462Y_agg2x
PCTS="10 25 53 100"
...
OUT  = ROOT / "logs" / "final_hotstorage_results_8462Y_agg2x"
TSV  = OUT / "results_final_hotstorage_8462Y_agg2x.tsv"
LABEL = "8462Y  4cpu agg2x (final)"
LLC_KIB = 8192
def stat(d, key):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8462Y+ final HotStorage (4 CPU, LLC=8MiB, agg=2×LLC, all jobs at once) ====="
