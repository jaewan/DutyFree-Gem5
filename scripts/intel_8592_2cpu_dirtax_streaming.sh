#!/usr/bin/env bash
# Intel Xeon 8592+ (Emerald Rapids) — 2 CPU: DirTax + STREAMING (combined)
# LLC = 2 HNF × 5MiB = 10MiB ; aggressor = 4 × LLC(total) = 40MB each
...
OUT=$ROOT/logs/intel_8592_2cpu_dirtax_streaming
...
OUT  = ROOT / "logs" / "intel_8592_2cpu_dirtax_streaming"
TSV  = OUT / "results_intel_8592_2cpu_dirtax_streaming.tsv"
LABEL = "8592   2cpu"
LLC_KIB = 10240
def ticks(d):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8592+ DirTax+STREAMING (2 CPU, LLC=10MiB, agg=4×LLC, max ${MAXJOBS} jobs) ====="
