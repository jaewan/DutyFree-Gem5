...
OUT=$ROOT/logs/final_hotstorage_results_8462Y_agg4x
PCTS="10 25 53 100"
...
OUT  = ROOT / "logs" / "final_hotstorage_results_8462Y_agg4x"
TSV  = OUT / "results_final_hotstorage_8462Y_agg4x.tsv"
LABEL = "8462Y  4cpu agg4x (final)"
LLC_KIB = 8192
def stat(d, key):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8462Y+ final HotStorage (4 CPU, LLC=8MiB, agg=4×LLC, all jobs at once) ====="
