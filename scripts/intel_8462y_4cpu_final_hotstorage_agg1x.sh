...
# LLC = 4 HNF × 2MiB = 8MiB ; aggressor = 1 × LLC(total) = 8MB each
...
OUT=$ROOT/logs/final_hotstorage_results_8462Y_agg1x
PCTS="10 25 53 100"
...
OUT  = ROOT / "logs" / "final_hotstorage_results_8462Y_agg1x"
TSV  = OUT / "results_final_hotstorage_8462Y_agg1x.tsv"
LABEL = "8462Y  4cpu agg1x (final)"
LLC_KIB = 8192
def stat(d, key):
    try:
...
run_all() {
    mkdir -p $OUT
    echo "===== Intel 8462Y+ final HotStorage (4 CPU, LLC=8MiB, agg=1×LLC, all jobs at once) ====="
